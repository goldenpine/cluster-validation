...
if [ $MRV == "2" ] ; then
   ...
   tdisks=$(maprcli dashboard info -json |grep total_disks |egrep -o '[0-9]+' |awk '{print int($1+0.5)}')
   ...
   filesPerDisk=1
   mapDisk=`echo "scale=2; 1 / $filesPerDisk" | bc`; echo Number of disks per Map task: $mapDisk
